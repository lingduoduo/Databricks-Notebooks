...
ratings = spark.sql(f"""select _token_associate_id as user_id, click_object_id as item_id, sum(click) as rating 
...
ratings_data = ratings.select('user_id', 'item_id').collect()

for i in range(n):
    user_id = ratings_data[i][0]
    item_id = ratings_data[i][1]
    if user_id not in map_users_id:
        map_users_id[user_id] = len(map_users_id) + 1
    if item_id not in map_items_id:
        map_items_id[item_id] = len(map_items_id) + 1
...
def map_user_id(user_id):
    return map_users_id.get(user_id)

def map_item_id(item_id):
    return map_items_id.get(item_id)

map_user_id_udf = udf(map_user_id, IntegerType())
map_item_id_udf = udf(map_item_id, IntegerType())

# Apply the UDFs to the DataFrame
ratings = ratings.withColumn("user_id", map_user_id_udf("user_id")) \
                     .withColumn("item_id", map_item_id_udf("item_id")) \
